...
<form method="post">
        <div class="shopping-cart" style="margin-top: 50px">            <?php                
            ...
            $res = $con->query($sel);
                while ($row=$res->fetch_assoc()) {
                    $selPr = "select * from tbl_products where product_id='" .$row["product_id"]. "'";
                    $respr = $con->query($selPr);
                    if ($rowpr=$respr->fetch_assoc()) {
                        $selstock = "select sum(product_quantity) as stock from tbl_products where product_id='".$rowpr["product_id"]."'";
            ...
            <div class="product">
                <div class="product-image">
                    <img
                        src="../Assests/Files/ProductsDocs/<?php echo $rowpr["product_photo"] ?>"
                        />
                </div>
                ...
                <select id="<?php echo $row["cart_id"]?>" style="width:90px">
					...
					{
						?>
                        <option <?php if($row["cart_quantity"]==$k){ echo "selected";} ?> value="<?php echo $k ?>"><?php echo $k ?>Kg</option>
						<?php
					}
					?>
                </select>
        ...
        $(".product-quantity select").change(function() {
			...
			$.ajax({
                url: "../Assests/Ajax/AjaxCart.php?action=Update&id=" + this.id + "&qty=" + this.value
            });
            updateQuantity(this);
        ...
        $(".product-removal button").click(function() {
            ...
            $.ajax({
                url: "../Assests/Ajax/AjaxCart.php?action=Delete&id=" + this.value
            });
            removeItem(this);
        });
